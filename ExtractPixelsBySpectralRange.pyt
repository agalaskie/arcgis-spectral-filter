...
class SpectralRangeTool(object):
    ...
    def getParameterInfo(self):
        ...
        for i in range(1, 21):
            ...
            add_more = arcpy.Parameter(
                name=f"add_{i}",
                displayName="Additional Band",
                datatype="GPBoolean",
                parameterType="Optional",
                direction="Input"
            )
    ...
    def updateParameters(self, parameters):
        ...
        for i in range(4, 77, 4):
            if parameters[i].value == True:
                ...
                if i+4 < len(parameters):
                    parameters[i+4].enabled = True
            else:
                for j in range(i+1, min(i+5, len(parameters))):
    ...
    def execute(self, parameters, messages):
        ...
        desc = arcpy.Describe(in_ras_path)
        ...
        for i in range(1, 81, 4):
            ...
            if parameters[i].valueAsText:
                ...
                band_ras = Raster(in_ras_path + "/" + band_name)
        ...
        out = SetNull(final_mask == 0, 1)
        out.save(out_ras_path)
